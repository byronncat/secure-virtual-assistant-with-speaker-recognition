 ...
 * WebM/Opus container. Matches the backend pipeline:
 *
 *   Browser (raw PCM @ device rate, e.g. 48kHz) -> FastAPI -> resample to 16kHz
...
export function useVoiceRecorder({
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // Deliberately not connected to audioContext.destination to avoid mic feedback
